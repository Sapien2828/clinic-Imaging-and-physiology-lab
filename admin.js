window.addEventListener('DOMContentLoaded', () => {
  ...
  appId: "1:568457688933:web:2eee210553b939cf39538c"
    ...
    function handleResetAll(isAutomatic = false) {
        const confirmReset = () => {
    ...
    async function setPatientToExamining(patient, specificRoom) {
        ...
        if (proceed) {
            ...
            if (patient.isExamining && patient.assignedExamRoom && patient.assignedExamRoom !== specificRoom) {
                // If moving from another room, we don't need to do anything extra as the new assignment overrides
            }
    ...
    async function handleCancelLabReception(e) {
        ...
        if (confirm(`「${currentLabGroup}」の受付のみを取り消しますか？`)) {
            ...
            if (newLabs.length === 0) {
                ...
                await patientsCollection.doc(patientId).update(updateData);
            }
        }
    }
